...
to build it (which should take no more than a few seconds) and run from the build
directory.[^1]
...
to starting an application (alacritty is used in the example, but it could of
...
<img src="https://i.imgur.com/SWhEhuz.png">

# Configuration

labwc user configuration files are located at `~/.config/labwc/` with the
following four files forming the basis:
...
Create a `~/.config/labwc/menu.xml` as in the example below:
...
to `~/.config/labwc/autostart`. See example below using swaybg and waybar:
...
See further examples in [autostart](https://github.com/labwc/labwc/blob/master/docs/autostart)
